...
func TestRegisterNCloseJobPostings(t *testing.T) {
	t.Run("RegisterNCloseJobPostings", func(t *testing.T) {
		...
		grpcServer := gServer.NewDataProcessorServer(jobPostingRepo, tinit.InitCompanyRepo(t))

		pbJobPostings := samplePbJobPostings()

		for _, pbJobPosting := range pbJobPostings {
			grpcServer.RegisterJobPostingInfo(context.Background(), pbJobPosting)
		}
		...
		// CloseJobPostings
		grpcServer.CloseJobPostings(context.Background(), &processor_grpc.JobPostings{
